...
class Pump_View():
    ...
    def DoPlot(self, Model):
        ...
        :return:
        """
        "Used CHATGPT to help formulate the correct plotting code"
        ...
        # Plot Head Data
        axes.plot(Model.FlowData, Model.HeadData, 'o', color='black', markerfacecolor='white', label='Head')
        axes.plot(headx, heady, '--', color='black', label=f'Head($R^2$ = {headRSq:.3f})')

        # Twin axis for efficiency
        ax2 = axes.twinx()
        ax2.plot(Model.FlowData, Model.EffData, '^', color='black', markerfacecolor='white', label='Efficiency')
        ax2.plot(effx, effy, ':', color='black', label=f'Efficiency($R^2$ = {effRSq:.3f})')

        # Labels
        axes.set_xlabel(f'Flow Rate ({Model.FlowUnits})')
        axes.set_ylabel(f'Head ({Model.HeadUnits})')
        ax2.set_ylabel('Efficiency (%)')
        axes.set_title('Pump Performance Curves')
