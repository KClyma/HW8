"""
Pump Performance Analyzer (MVC Pattern)
Problem Statement:
Design and implement a Python application using the Model-View-Controller (MVC) design pattern to read, analyze,
and visualize pump performance data from a structured text file. The application provides a user-friendly GUI for
selecting pump data files, displaying key information, and plotting pump performance curves.
Features:
1. GUI Integration:
   - A PyQt-based interface allows users to browse for and select a `.txt` pump data file.
   - The file path is stored for easy repeat access.
2. Data Parsing:
   - Extracts pump metadata (e.g., name, flow units, head units) and performance data (flow rate, head, efficiency).
3. Curve Fitting:
   - Applies a quadratic fit to the head data and a cubic fit to the efficiency data using least squares regression.
   - Polynomial coefficients are stored and displayed.
4. Output Display and Plotting:
   - Displays pump name, units, and fit coefficients.
   - Plots both head and efficiency vs. flow rate with matplotlib:
     - X-axis: flow rate
     - Left Y-axis: head (ft)
     - Right Y-axis: efficiency (%)
     - Legends and R² values included
     - Clear plot title and axis labels
This program uses the MVC architecture to enforce separation between data modeling, user interaction, and visualization.
"""
#region imports
import numpy as np
...
class PumpCurve_GUI_Class(Ui_Form, qtw.QWidget):  #class for PumpCurve_GUI inherits from Ui_Form & qtw.QWidget
    ...
    def OpenFile(self):
        ...
        '''
        fname=qtw.QFileDialog.getOpenFileName(self, 'Open file', self.FilePath, "Text files (*.txt);;All files (*)") # use qtw.QFileDialog.getOpenFileName
        oTF=len(fname[0])>0
        if oTF:
