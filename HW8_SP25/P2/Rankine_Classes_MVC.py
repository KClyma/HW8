...
class rankineView():
    ...
    def selectQualityOrTHigh(self, Model=None):
        ...
        if self.rdo_Quality.isChecked():
            ...
            try:
                P = float(self.le_PHigh.text())
                P_bar = P if SI else P * UC.psi_to_bar
                tsat = Model.steam.getsatProps_p(P_bar).tsat
                T_display = tsat if SI else UC.C_to_F(tsat)
                self.le_TurbineInletCondition.setText(f"{T_display:.2f}")
                self.le_TurbineInletCondition.setEnabled(True)
            except:
                self.le_TurbineInletCondition.setText("Invalid P High")
    ...
    def setNewPHigh(self, Model=None):
        """
                 ...
                 :return:
                 """
        "Used CHATGPT to check my code for errors"
        SI = self.rb_SI.isChecked()
        try:
            P = float(self.le_PHigh.text())
            PCF = 1 if SI else UC.psi_to_bar
            satProps = Model.steam.getsatProps_p(P * PCF)
    ...
    def updateUnits(self, Model=None):
        ...
        :return:
        """
        "Used CHATGPT to help finish this code"
        ...
        self.le_PHigh.setText(f"{Model.p_high * pCF:.2f}")
        self.le_PLow.setText(f"{Model.p_low * pCF:.2f}")

        #Step 2. Update THigh if it is not None
        if not self.rdo_Quality.isChecked():
            if Model.t_high is not None:
                T_display = Model.t_high if Model.SI else UC.C_to_F(Model.t_high)
                self.le_TurbineInletCondition.setText(f"{T_display:.2f}")

        #Step 3. Update the units for labels
        self.lbl_H1Units.setText("kJ/kg" if Model.SI else "BTU/lb")
        self.lbl_H2Units.setText("kJ/kg" if Model.SI else "BTU/lb")
        self.lbl_H3Units.setText("kJ/kg" if Model.SI else "BTU/lb")
        self.lbl_H4Units.setText("kJ/kg" if Model.SI else "BTU/lb")
        self.lbl_TurbineWorkUnits.setText("kJ/kg" if Model.SI else "BTU/lb")
        self.lbl_PumpWorkUnits.setText("kJ/kg" if Model.SI else "BTU/lb")
        self.lbl_HeatAddedUnits.setText("kJ/kg" if Model.SI else "BTU/lb")
