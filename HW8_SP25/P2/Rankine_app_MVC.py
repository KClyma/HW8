"""
Rankine Cycle Simulator (MVC Pattern)
Problem Statement:
Develop a graphical Rankine cycle simulator using the Model-View-Controller (MVC) design pattern. The application allows
users to analyze the thermal efficiency of a Rankine cycle given input conditions and unit system preferences.
All logic and updates are coordinated internally by the controller and reflected in the GUI through the view.
This program uses a modular architecture with:
    - rankineModel: stores all thermodynamic state data and system properties.
    - rankineController: manages logic, updates the model from inputs, and refreshes the view.
    - rankineView: displays updated outputs, labels, and plots based on the model.
Core Functional Requirements:
1. When the user selects the T High radio button:
   - Automatically populate the Turbine Inlet temperature field with the saturation temperature at P High.
2. When the user edits P High or P Low:
   - Update the corresponding saturated property label.
   - If T High mode is active, update the turbine inlet temperature accordingly.
3. When the user switches between SI and English units:
   - All input fields, output labels, and plots update immediately to reflect the selected units.
   - The only function connected to the clicked signal of the unit radio buttons must be:
        self.RC.updateUnits()
All modifications to the GUI must be routed through the controller and view in accordance with the MVC architecture.
"""
#region imports
import sys
